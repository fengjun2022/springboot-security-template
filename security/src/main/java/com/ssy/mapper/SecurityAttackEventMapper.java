...
public interface SecurityAttackEventMapper {
    ...
    @Insert("INSERT INTO security_attack_event " +
            "(ip, attack_type, path, method, endpoint_id, username, app_id, user_agent, referer, query_string, " +
            "request_body_sample, request_body_hash, risk_score, block_action, block_reason, suggested_action, create_time) " +
            "VALUES (#{ip}, #{attackType}, #{path}, #{method}, #{endpointId}, #{username}, #{appId}, #{userAgent}, #{referer}, #{queryString}, " +
            "#{requestBodySample}, #{requestBodyHash}, #{riskScore}, #{blockAction}, #{blockReason}, #{suggestedAction}, #{createTime})")
